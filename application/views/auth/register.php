  ...
  <div class="login-card">
    ...
    <div class="login-body">
      ...
      <?php if($this->session->flashdata('success')): ?>
        <div class="alert alert-success small mb-3"><?= $this->session->flashdata('success') ?></div>
      <?php endif; ?>

      <form action="<?= base_url('auth/register') ?>" method="post">
        <div class="mb-3">
          <label class="form-label">VA / Username</label>
          <input type="text" name="username" class="form-control" placeholder="Masukkan VA" required>
        </div>
        <div class="mb-3">
    ...
    <div class="login-footer mt-3">
      <small>Sudah punya akun? <a href="<?= base_url('auth') ?>" class="text-danger">Masuk di sini</a></small>
    </div>
  </div>
</div>
